...
import type { FeedbackReport, FeedbackSummary } from '@/features/feedback/services/fetchFeedback';

// ─── Types ──────────────────────────────────────────

type Tab = 'general' | 'trades' | 'legal' | 'integrations' | 'roles' | 'audit' | 'feedback';
...
const BASE_TABS: { key: Tab; label: string; icon: React.ReactNode; proOnly?: boolean; adminOnly?: boolean }[] = [
  ...
  { key: 'feedback', label: 'Feedback', icon: <IconChat />, adminOnly: true },
];

// ─── Component ──────────────────────────────────────

export function SettingsPage({ projectSettings, planId, userRole, feedbackReports = [], feedbackSummary, newFeedbackCount = 0 }: Props) {
  const [activeTab, setActiveTab] = useState<Tab>('general');
  const isStarter = planId === 'starter';
  const isGcAdmin = userRole === 'gc_admin';

  const defaultSummary: FeedbackSummary = { new: 0, reviewing: 0, in_progress: 0, resolved: 0 };
  const TABS = BASE_TABS.filter((t) => !t.adminOnly || isGcAdmin);
      ...
      <div className="flex-1 min-w-0">
        {isLocked ? (
          ...
          <>
            ...
            {activeTab === 'feedback' && isGcAdmin && (
              <FeedbackSection
                reports={feedbackReports}
                summary={feedbackSummary ?? defaultSummary}
              />
            )}
          </>
        )}
